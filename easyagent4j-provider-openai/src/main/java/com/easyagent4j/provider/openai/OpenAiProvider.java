...
/**
 * OpenAI Provider实现 — 使用JDK HttpClient调用OpenAI兼容API。
 * 支持OpenAI/DeepSeek/Qwen/GLM/Moonshot等兼容OpenAI协议的模型。
 */
public class OpenAiProvider implements LlmProvider {
    ...
    public void stream(ChatRequest request, Consumer<ChatResponseChunk> callback) {
        try {
            ...
            Thread.currentThread().interrupt();
            throw new RuntimeException("Failed to stream OpenAI API", e);
        }
    }
            ...
            if (choices != null && choices.isArray() && choices.size() > 0) {
                ...
                if (delta != null) {
                    ...
                    if (toolCalls != null && toolCalls.isArray()) {
                        ...
                        // Note: ChatResponseChunk doesn't have setToolCalls method
                        // Tool calls in streaming will be handled at the agent level
                    }
                }
